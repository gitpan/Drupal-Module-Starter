...
use strict;
use YAML;
use Drupal::Module::Starter::4_6_2;
use File::Path;
...
sub new {
	...
	$self->{cfg}->{drupal_version} = '4.6.2' unless $self->{cfg}->{drupal_version};
...
";
	...
	# add standard stubs for _name , _perms, 
   $self->{cfg}->{hook_node_name} = 1;
   $self->{cfg}->{hook_perm} = 1;
   
   # if we have any page callbacks defined, we need to create a hook_menu that knows 
   # about them
   my $has_pages = 0; 
   my @pages = (grep { /^page_/ } keys %{$self->{cfg}}); 
   $has_pages = 1 if scalar(@pages);
   if($has_pages) {
   	 $self->{cfg}->{hook_menu} = 1;
   	 
   	 # make a hook_menu that knows about the pages
   	 #$self->{stubs}->{hook_menu} = $self->{stubs}->{clean_menu};
   	 #my $itemlist;
   	 #for(@pages) {
   	 #   $_ =~ s/\s+//g;
   	 #$itemlist .= '$items[] = array("path" => '.$_.' 
   	 #}
...
Changelog - Version 0.1 (autogenerated $now)
...
Copyright 2005 Steve McNabb, All Rights Reserved.
